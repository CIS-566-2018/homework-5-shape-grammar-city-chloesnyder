...
void main()
{
    // Material base color (before shading)
        ...
        vec4 diffuseColor = vec4(116.0/255.0, 119.0/255.0, 124.0/255.0,1);
        // texture windows on x and z planes

     //   vec2 uv = convertToUV(fs_Pos);
/*
        float x = fs_Pos.x;
        float y = fs_Pos.z;
        float dx = .05;
        ...
        if(fs_Nor == vec4(1,0,0,0) || fs_Nor == vec4(-1,0,0,0) || fs_Nor == vec4(0,0,1,0) || fs_Nor == vec4(0, 0, -1, 0))
        {
           ...
           /* if((x > -.25 - dx && x < -.25 + dx) || (x > -dx && x < dx) || (x > .25 - dx && x < .25 + dx))
            ...
            } else if((y > -.25 - dy && y < -.25 + dy) || (y > -dy && x < dy) || (y > .25 - dy && y < .25 + dy))
            {
                if((x > -.25 - dx && x < -.25 + dx) || (x > -dx && x < dx) || (x > .25 - dx && x < .25 + dx))
                {
